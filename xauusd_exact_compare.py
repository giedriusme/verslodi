...
def mk(g, signal_idx, direction, family, tp, sl):
    ...
    return {'date':str(g.iloc[0].local_date),'year':int(g.iloc[0].year),'family':family,'direction':direction,'entry':entry,'tpv':tp,'slv':sl,
...
def eval_trade(c):
    ...
    if c['direction']=='LONG': itp=np.flatnonzero(hi>=e+tp); isl=np.flatnonzero(lo<=e-sl); eod=float(cl[-1]-e)
    else: itp=np.flatnonzero(lo<=e-tp); isl=np.flatnonzero(hi>=e+sl); eod=float(e-cl[-1])
...
def stats(cs):
    ...
    r=pd.DataFrame(rows); p=r.pnl.to_numpy(float); pos=int((p>0).sum()); neg=int((p<0).sum()); zero=int((p==0).sum())
    ...
    return {'n':len(r),'positive':pos,'negative':neg,'zero':zero,'positive_rate':float(pos/len(r)),'negative_rate':float(neg/len(r)),
            ...
            'net':float(p.sum()),'expectancy':float(p.mean()),'profit_factor':float(gains/losses) if losses>0 else None,'max_drawdown':float(dd.max()) if len(dd) else 0.,
            'max_loss_streak':ls,'loss_streak_start':lsd,'loss_streak_end':led,'max_win_streak':ws,'win_streak_start':wsd,'win_streak_end':wed,'years':years},r

def main():
    df=bt.load_data(); df['mod']=(df.hour*60+df.minute).astype(int)
    close_carry=[]; close_post10=[]; close_10_16=[]; extreme=[]; mom=[]
    for day,g0 in df.groupby('local_date',sort=True):
        ...
        if len(asia)>=420:
            ...
            post16=g[(g['mod']>=600)&(g['mod']<960)]; idx,d=first_close_break(post16,ah,al)
            if d is not None:
                c=mk(g,idx,d,'asia_close_10_16',20.,20.)
                if c:close_10_16.append(c)
            s=g[g['mod']==600]
            if not s.empty and ah>al:
                r=s.iloc[0]; rel=(float(r.close_mid)-al)/(ah-al); idx=int(s.index[0]); d='LONG' if rel>=.9 else ('SHORT' if rel<=.1 else None)
                if d:
        ...
        if len(w)>=42:
            ...
            if abs(mv)>=8.:
                d='LONG' if mv>0 else 'SHORT'; c=mk(g,int(w.index[-1]),d,'10_11_momentum_continuation',35.,30.)
                if c:mom.append(c)
    result={}
    for name,cs in [('asia_close_carry_pre10',close_carry),('asia_close_post10_only',close_post10),('asia_close_10_16',close_10_16),('asia_extreme_continuation',extreme),('10_11_momentum_continuation',mom)]:
